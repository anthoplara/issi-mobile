...
class LoginOtherBloc {
  ...
  fetchResponse(String username, String name, String email, String photo) async {
    _isStreaming = true;
    antDataSink.add(ApiResponse.loading(''));
    String userId = localData.read(KeyStorage.userId) ?? "0";
    try {
      LoginModel dataResponse = await _antLoginRepository.fetchOtherLogin({
        'username': username,
        'nama': name,
        'email': email,
        'url_photo': photo,
      ...
      if (_isStreaming) {
        if (!(dataResponse.error ?? true)) {
          if (dataResponse.data!.isNotEmpty) {
            ...
            await localData.write(KeyStorage.lastUserName, "");
